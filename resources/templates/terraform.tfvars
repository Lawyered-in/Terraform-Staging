...
codepipelines = {
  ...
  core-platform-fe = {
    ...
    custom_build_commands = [
      "echo Build started on `date`",
      "echo Injecting NODE_OPTIONS heap limit to Dockerfile...",
      "sed -i 's|RUN npm run build|ENV NODE_OPTIONS=\"--max-old-space-size=4096\"\\nRUN npm run build|g' Dockerfile",
      "echo Building the Docker image...",
      "docker build --build-arg VITE_API_URL=$${VITE_API_URL} -t $REPOS_URL:latest .",
      "docker tag $REPOS_URL:latest $REPOS_URL:$IMAGE_TAG"
    ]
    tags = {
      Environment = "stage"
