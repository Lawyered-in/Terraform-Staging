...
variable "codepipelines" {
  type = map(object({
    ...
    custom_deploy_commands     = optional(list(string))
    enable_security_scan       = optional(bool, false)
    enable_gated_deploy        = optional(bool, false)
    build_compute_type         = optional(string, "BUILD_GENERAL1_SMALL")
    security_scan_compute_type = optional(string, "BUILD_GENERAL1_SMALL")
    critical_threshold         = optional(number, 3)
    medium_threshold           = optional(number, 5)
    low_threshold              = optional(number, 10)
    tags                       = optional(map(string), {})
  }))
